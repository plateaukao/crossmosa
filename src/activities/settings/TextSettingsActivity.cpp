...
void TextSettingsActivity::render(RenderLock&&) {
  ...
  switch (tab_) {
    ...
    case Tab::Style: {
      ...
      static constexpr StrId ROW_NAME_IDS[STYLE_ROWS] = {StrId::STR_BOLD_TEXT, StrId::STR_FOCUS_READING,
...
void TextSettingsActivity::confirmStyleRow(int row) {
  switch (static_cast<StyleRow>(row)) {
    case StyleRow::BoldText:
      SETTINGS.boldBodyText = !SETTINGS.boldBodyText;
      ParsedText::setBoldBodyText(SETTINGS.boldBodyText != 0);  // 預覽與回到閱讀器後的重排都吃這個旗標
      break;
...
std::string TextSettingsActivity::styleValueText(int row) const {
  switch (static_cast<StyleRow>(row)) {
    case StyleRow::BoldText:
      return SETTINGS.boldBodyText ? tr(STR_STATE_ON) : tr(STR_STATE_OFF);
    case StyleRow::FocusReading:
